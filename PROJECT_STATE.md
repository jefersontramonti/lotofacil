...
| ID | Requisito (resumo) | Pri | Fase | Status | Branch/PR |
|---|---|---|---|---|---|
| RF-05.1 | Buscar resultado oficial na API da Caixa | M | F1 | concluído | `ResultadoApi`/`ResultadoRepositoryImpl` (`:core:data`), API real da Caixa (`servicebus2.caixa.gov.br`), verificado com chamada de rede real no emulador. **Achado de auditoria de segurança (security-auditor, 2026-08-22), corrigido:** `ResultadoDto.paraDominio()` (`ResultadoMapper.kt`) não validava a forma do payload — quantidade de dezenas diferente de 15, dezena fora de 1–25 ou repetida seriam aceitas e usadas na conferência do usuário, gerando acerto/erro incorreto (pior que um erro de rede). Adicionado `validarDezenasSorteadas()`, que lança (não silencia) se o payload vier malformado; a exceção não é `IOException`/`HttpException`, então não é retentada por `comRecuoExponencial` — cai direto no tratamento genérico de falha (`ConferenciaUiState.Falha`). Testado em `ResultadoMapperTest.kt` (quantidade errada, fora do intervalo, repetida) e verificado com a API real no emulador (payload real passa, sem regressão) |
...
| RF-05.3 | Conferir automaticamente todos os palpites do concurso | M | F2 | concluído | roda ao entrar na `TelaConferencia` (`ConferenciaViewModel.aoEntrar`), não em background/WorkManager — ver pendência de RNF-02.3. **Pull-to-refresh:** `PullToRefreshBox` (Material 3) envolve o conteúdo; puxar pra baixo chama o mesmo `onTentarNovamenteClick` (`aoTentarNovamente`) já usado pelo botão de erro — `isRefreshing = uiState is Carregando`. Verificado no emulador: swipe-down dispara uma nova requisição real à API da Caixa (log do OkHttp confirma) |
...
| RF-09.6 | Liberar Pro imediatamente após compra; revogar ao fim | M | F3 | concluído | `EstadoDaAssinatura` deriva sempre de `BillingClient.queryPurchasesAsync` (nunca uma flag local solta) — se o Billing para de devolver a compra, o estado volta pra `Gratuito` sozinho. `PurchasesUpdatedListener` libera na hora após `launchBillingFlow`. **Achados de auditoria de segurança (security-auditor, 2026-08-22), corrigidos:** (1) `queryPurchasesAsync` só rodava no `init` do repositório e em `restaurarCompras()` (botão explícito) — sem observar o app voltar do background, uma revogação (cancelamento/reembolso feito na Play Store sem fechar o app) podia deixar o Pro preso até o processo morrer. Corrigido com `ProcessLifecycleOwner`/`DefaultLifecycleObserver.onStart` (`AssinaturaRepositoryImpl`, nova dependência `androidx.lifecycle:lifecycle-process` em `:core:data`) — resincroniza as compras toda vez que o app volta ao primeiro plano. (2) `acknowledgePurchase` descartava o `BillingResult`, então uma falha (rede instável) passava em silêncio — compra não reconhecida em 3 dias é reembolsada automaticamente pelo Google. Corrigido com `confirmarCompra()`: tenta até 3 vezes com atraso entre tentativas; se ainda assim falhar, a próxima `processarCompras()` (abertura do app, restaurar, ou volta do background) tenta de novo. Sem teste automatizado (mockar `BillingClient`, classe final do SDK, exigiria Mockito/Robolectric — infraestrutura de teste que o projeto não tem hoje); verificado só por leitura de código e build/lint/ktlintCheck/app rodando no emulador sem crash |
...
| RNF-04.1 | HTTPS obrigatório, texto claro desabilitado no manifesto          | concluído — `android:usesCleartextTraffic="false"` + base url HTTPS. **Achado de auditoria de segurança (security-auditor, 2026-08-22), corrigido:** `android:allowBackup` não estava declarado (padrão do Android é `true`) — nome e data de nascimento saíam do aparelho pelo Auto Backup sem consentimento explícito. Adicionado `android:allowBackup="false"` no `AndroidManifest.xml`; verificado no emulador (`dumpsys package` confirma a flag `ALLOW_BACKUP` ausente do app instalado) |
...
| RNF-04.4 | Conformidade LGPD (privacidade, consentimento, exclusão de dados) | parcial — **achado de auditoria de segurança (security-auditor, 2026-08-22), corrigido:** não existia nenhum caminho de exclusão de dados no app. Adicionado "Excluir meus dados" em `TelaPerfil` (seção "Seus dados", com diálogo de confirmação) → `PerfilViewModel.aoConfirmarExclusaoDeDados()` apaga perfil/crenças/notificações (`PreferenciasRepository.excluirTudo()`, limpa o DataStore inteiro) e todos os palpites (`PalpiteRepository.excluirTodos()`, `DELETE FROM palpites`), cancela lembretes/notificações agendados (senão ficariam órfãos) e navega de volta pro onboarding (`popUpTo` o grafo inteiro). Cobre os dois únicos dados pessoais coletados (nome, nascimento). Testado em `PerfilViewModelTest` e verificado no emulador ponta a ponta, incluindo restart a frio confirmando que a exclusão persiste (app volta pro onboarding). Falta ainda: privacidade/consentimento não são parte deste achado, resto do requisito segue não iniciado |
...
| — | Timeout explícito no cliente de rede — achado de auditoria de segurança (security-auditor, 2026-08-22) | concluído — `NetworkModule.fornecerOkHttpClient()` não declarava `connectTimeout`/`readTimeout`, rodando só nos defaults implícitos do OkHttp; um comportamento de rede atípico deixaria o `ResultadoSorteioWorker` pendurado sem limite documentado. Adicionado 15s pros dois (`TIMEOUT_DE_REDE`), testado em `NetworkModuleTest` |
| — | R8/minify e shrinkResources no release — achado de auditoria de segurança (security-auditor, 2026-08-22) | concluído — `app/build.gradle.kts` não tinha `buildTypes { release { ... } }` nenhum; o release saía sem shrink, com nomes de classe/método (incluindo o caminho de validação de Pro/Billing) legíveis num decompilador. Adicionado `isMinifyEnabled`/`isShrinkResources` + `app/proguard-rules.pro` (regras pra kotlinx.serialization manter os `@Serializable`/serializers do app; Room/Hilt/Retrofit/Compose/Billing/Ads já trazem as próprias). Verificado com `assembleRelease` real, assinado localmente com o keystore de debug só pra teste (nunca para publicar) e rodado ponta a ponta no emulador: onboarding → cálculo de signo → geração de palpite (Room) → Home → Perfil (Billing/Assinatura carrega) → Conferência (rede + kotlinx.serialization) — sem crash em nenhum passo |
