...
// Rotas que mostram a barra: as quatro abas de conteúdo, mais Detalhe,
// Desdobramentos (alcançados a partir da Home, sem aba própria — RF-04) e
// "Suas crenças" (alcançada a partir do Perfil, sem aba própria — RF-07.2).
...
private fun abaAtivaPara(rota: String?): String? =
    when (rota) {
        ...
        Rotas.PERFIL, Rotas.PERFIL_CRENCAS -> "perfil"
        else -> null
    }
...
fun BarraDeNavegacaoDoApp(
    ...
    val perfilAtivo = abaAtiva == "perfil"
        ...
        itens =
            listOf(
                ...
                ItemDeNavegacao(
                    ...
                    descricaoDoEstado =
                        if (perfilAtivo) {
                            stringResource(id = R.string.nav_item_descricao_ativo, rotuloPerfil)
                        } else {
                            rotuloPerfil
                        },
                    icone = if (perfilAtivo) PhosphorIcons.Fill.User else PhosphorIcons.Regular.User,
                    ativo = perfilAtivo,
                    onClick = {
                        if (!perfilAtivo) {
